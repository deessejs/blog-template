...
4. Write    — build .claude/plans/{n}.md from PLAN_TEMPLATE.md
...
cp .claude/skills/spec/PLAN_TEMPLATE.md ".claude/plans/{n}.md"
```
...
**If rejected:**
> "Spec rejected. Branch `{slug}` still exists with the spec at `.claude/plans/{n}.md`. Say the word and I'll delete it, or keep it open for later."

## §6 — Push

```bash
git add .claude/plans/{n}.md
...
- **Spec is draft** → "A draft spec exists. Review it at `.claude/plans/{n}.md` on `{slug}`, or run `/spec #{n}` again to overwrite it."
